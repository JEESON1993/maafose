...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  void placeorder(itemtotal, totalpayable, deliverycharge, paymentType,
      instructionController) async {
    ...
    if (paymentType == 'online') {
      ...
      Map<String, dynamic> data = {
        "secondaryNumber": _instrseondrynumberController,
        "note": instructionController,
  ...
  TextEditingController _instrseondrynumberController;
  // ignore: unused_field
  String _radioVal;
  @override
  Widget build(BuildContext context) {
    ...
    double itemtotal = getcartmodel.cart
        .map((item) => item["packingCharge"] * item["qty"])
        .fold(0, (prev, amount) => prev + amount);
    double gst = ((itemtotal * 5) / 100);
    double totalpayable = widget.discountedTotal != null
        ? widget.tip != null
            ? double.parse(widget.tip) +
                ...
                charge.toDouble() +
                gst
            : widget.discountedTotal + charge.toDouble() + gst
        : widget.tip != null
            ? widget.tip == ''
                ? itemtotal + charge.toDouble() + gst
                : double.parse(widget.tip) + itemtotal + charge.toDouble() + gst
            : itemtotal + charge.toDouble() + gst;
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    Padding(
                                      ...
                                      child: TextFormField(
                                        controller:
                                            _instrseondrynumberController,
                                        minLines: 1,
                                        maxLines: 4,
                                        keyboardType: TextInputType.multiline,
                                        decoration: InputDecoration(
                                          hintText: 'Add a seconday Number ',
                                          hintStyle: kText10,
                                          border: OutlineInputBorder(
                                            borderRadius: BorderRadius.all(
                                                Radius.circular(10.0)),
                                  ...
                                  child: Padding(
                                    ...
                                    child: Container(
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () {
                                          if (charge != 0) {
                                            placeorder(itemtotal, totalpayable,
                                                charge, _value, note);
                                          } else {
                                            showSnackBar(
                                              duration:
                                              ...
                                              context: context,
                                              message:
                                                  "Order Cannot be placed,Please correct your delivery address",
                                            );
                                          }
